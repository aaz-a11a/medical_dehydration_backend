...
// CountUserRequestsByStatus считает количество заявок пользователя по статусу
func (r *Repository) CountUserRequestsByStatus(userID uint, status string) (int64, error) {
	var cnt int64
	err := r.db.Model(&ds.DehydrationRequest{}).
...
func (r *Repository) ListAllRequestsWithFilters(status *string, from, to *string) ([]ds.DehydrationRequest, error) {
	var list []ds.DehydrationRequest
	tx := r.db.Model(&ds.DehydrationRequest{}).
		Where("status NOT IN ?", []string{"удален", "удалён", "черновик"}).
		Preload("User").
		Preload("Moderator")
	if status != nil && *status != "" {
		tx = tx.Where("status = ?", *status)
